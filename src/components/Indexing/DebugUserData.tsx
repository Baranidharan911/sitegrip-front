...
export default function DebugUserData() {
  ...
  const testBackend = async () => {
    try {
      ...
      alert(`✅ Backend Status: ${res.status}\nResponse: ${JSON.stringify(data, null, 2)}`);
    } catch (error) {
  ...
  const testGoogleStatus = async () => {
    try {
      ...
      if (!userId) return alert('No user ID found. Click "Test Login" first.');
    ...
    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="space-x-2">
          <button onClick={refreshData} className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">Refresh</button>
          <button onClick={testLogin} className="px-3 py-1 text-sm bg-green-600 text-white rounded hover:bg-green-700">Test Login</button>
          <button onClick={testBackend} className="px-3 py-1 text-sm bg-purple-600 text-white rounded hover:bg-purple-700">Test Backend</button>
          <button onClick={testGoogleStatus} className="px-3 py-1 text-sm bg-pink-600 text-white rounded hover:bg-pink-700">Test Google Status</button>
          <button onClick={testGoogleOAuth} className="px-3 py-1 text-sm bg-pink-600 text-white rounded hover:bg-pink-700">Test Google OAuth</button>
          <button onClick={clearData} className="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700">Clear</button>
        </div>
      </div>
