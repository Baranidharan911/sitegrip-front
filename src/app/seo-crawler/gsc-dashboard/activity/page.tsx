...
// Enhanced Google Search Console style activity chart component with dark mode support
    ...
    return (
      <div className="flex items-center justify-center h-full text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400">No activity data available</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Activity metrics will appear here when data is available</p>
        </div>
      </div>
    );
  }
...
export default function GSCActivityPage() {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      // Convert pages to activity format
      const activityData = (pagesResponse.data.pages || []).map((page: any) => ({
        id: page.url,
        type: page.indexed ? 'indexed' : 'not_indexed',
        status: page.coverageState || 'unknown',
        url: page.url,
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'indexed': return <CheckCircle className="w-4 h-4 text-green-600 dark:text-green-400" />;
      case 'not_indexed': return <Clock className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />;
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'Submitted and indexed': return 'Indexed';
      case 'Discovered – currently not indexed': return 'Discovered';
      case 'Crawled – currently not indexed': return 'Crawled';
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-normal text-gray-900 dark:text-white">Activity</h1>
              <p className="text-sm text-gray-600 dark:text-gray-400">Monitor your site's recent activity</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Total Activities</h3>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Indexed</h3>
              <CheckCircle className="w-6 h-6 text-green-600 dark:text-green-400" />
            </div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white">
              {formatNumber(activities.filter(a => a.type === 'indexed').length)}
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-gray-900 dark:text-white">
              {formatNumber(activities.filter(a => a.type === 'not_indexed').length)}
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
              Pending indexing
            </p>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Errors</h3>
              <XCircle className="w-6 h-6 text-red-600 dark:text-red-400" />
            </div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white">
              {formatNumber(activities.filter(a => a.type === 'error').length)}
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
              With errors
            </p>
          ...
          <div className="h-80">
            <GSCActivityChart
              data={activities.map((activity, index) => ({
                ...
                value: activity.details.clicks || 0
              }))}
              color="#1a73e8"
              height={320}
              title="Activity"
        ...
        <div className="mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              {['all', 'indexed', 'not_indexed', 'error'].map((type) => (
                <button
                  key={type}
          ...
          <div className="space-y-3">
            {filteredActivities.slice(0, 20).map((activity, index) => (
              <div key={index} className="flex items-center justify-between p-4 border border-gray-100 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-medium text-sm truncate text-gray-900 dark:text-white">{activity.url}</p>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        activity.type === 'indexed' 
                          ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300'
                          : activity.type === 'not_indexed'
                ...
                <div className="flex items-center gap-4">
                  {activity.details.clicks > 0 && (
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{activity.details.clicks}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Clicks</p>
                    </div>
                  )}
                  
                  {activity.details.impressions > 0 && (
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{activity.details.impressions}</p>
